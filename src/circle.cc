...
void Circle::Init(Config* config, SDL_Renderer* renderer, Uint32 ticks_finish) {
  ...
  bg_color_ = config_->getProgressBgColor();
  line_width_ = config_->getProgressWidth();
  radius_ = config_->getWidth() / 2 - (line_width_ / 2) - 2;
  x_ = config_->getWidth() / 2;
  y_ = x_;
}

void Circle::RenderPre() {
  int startDuration = config_->getProgressStartDuration();
...
void Circle::RenderPost() {
	int duration = config_->getProgressEndDuration();
